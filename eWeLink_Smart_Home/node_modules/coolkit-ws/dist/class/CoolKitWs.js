...
class CoolKitWs {
    init(config) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (config.userAgent === 'app' && (!config.at || !config.apikey || !config.appid || !config.region)) {
                    throw new Error('app端长连接缺少必须参数！');
                }
                if (config.userAgent === 'device' && (!config.appid || !config.region || !config.apikey || !config.chipid || !config.deviceid)) {
                    throw new Error('设备端长连接缺少必须参数！');
                }
