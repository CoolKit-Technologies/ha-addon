...
class CoolKitWs {
    ...
    isWsExist() {
        if (CoolKitWs.ws) {
            WebSocket_1.default.connectConfig.debug && console.log(`WS:长连接已存在，状态为${CoolKitWs.ws.wsState === 'CLOSED' ? '已关闭' : '开启中'}`);
    ...
    on(method, callback) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (WebSocket_1.default.ws && ws) {
                if (method === 'reconnect') {
                    ws.updateCallback(`${enum_1.EOpenEventType.RECONNECT}`, (errorMsg) => callback(errorMsg));
                    eventEmitter_1.default.on(`${enum_1.EOpenEventType.RECONNECT}`, (errorMsg) => callback(errorMsg));
                    return;
                }
